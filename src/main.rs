...
const light_dir: Vec3f = Vec3f { x: 1., y: 1., z: 1. };
const       eye: Vec3f = Vec3f { x: 1., y: 1., z: 3. };
const    center: Vec3f = Vec3f { x: 0., y: 0., z: 0. };
const        up: Vec3f = Vec3f { x: 0., y: 1., z: 0. };
...
impl<'a> IShader for Shader<'a> {
    fn vertex(&mut self, iface: i32, nthvert: i32, view_bundle: &ViewBundle) -> Vec4f {
        self.varying_uv.set_col(nthvert, &vec![self.model.uv(iface, nthvert)[0], self.model.uv(iface, nthvert)[1]]);
        self.varying_nrm.set_col(nthvert, &proj_refactor(Into::<Vec<f32>>::into((view_bundle.ModelView.clone()).invert_transpose()*embed(&self.model.norm(iface, nthvert), Some(0.))), 3));
        let mut gl_Position = view_bundle.ModelView.clone()*embed(&self.model.vert(iface, nthvert), None);
        self.view_tri.set_col(nthvert, &proj_refactor(Into::<Vec<f32>>::into(gl_Position), 3));
    ...
    fn fragment(&self, bar: Vec3f) -> (bool, TGAColor) {
        ...
        let mut i = Vec3f::from_vec(&(AI.clone()*vec![self.varying_uv[0][1]-self.varying_uv[0][0], self.varying_uv[0][2]-self.varying_uv[0][0], 0.]));
        let mut j = Vec3f::from_vec(&(AI.clone()*vec![self.varying_uv[1][1]-self.varying_uv[1][0], self.varying_uv[1][2]-self.varying_uv[1][0], 0.]));
        let mut B = Matrix::new(Some(3), Some(3));
        B[0] = vec![i.normalize().to_owned()[0], i.normalize().to_owned()[1], i.normalize().to_owned()[2]];
        B[1] = vec![j.normalize().to_owned()[0], j.normalize().to_owned()[1], j.normalize().to_owned()[2]];
        B[2] = vec![bn[0], bn[1], bn[2]];
        B = B.transpose();

        let n = Vec3f::from_vec(&(B * vec![self.model.normal(&uv)[0], self.model.normal(&uv)[1], self.model.normal(&uv)[2]])).normalize().to_owned();
        let diff = 0f32.max(n*self.uniform_l);
        let r = (n*(n*self.uniform_l)*2. - self.uniform_l).normalize().to_owned();
...
fn main() {
    ...
    let view_bundle = ViewBundle { ModelView: ModelView.clone(), ViewPort: ViewPort.clone(), Projection: Projection.clone() };
