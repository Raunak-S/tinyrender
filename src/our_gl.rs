...
pub fn barycentric(tri: &[Vec2f; 3], P: &Vec2f) -> Vec3f {
    ...
    if ABC.det()<1e-3 { return Vec3f::new_args(-1., 1., 1.); }
...
pub trait IShader {
    fn sample2D(img: &TGAImage, uvf: &Vec2f) -> TGAColor {
        img.get((uvf[0]*img.get_width() as f32) as i32, (uvf[1]*img.get_height() as f32) as i32)
    }
    fn vertex(&mut self, iface: i32, nthvert: i32, view_bundle: &ViewBundle) -> Vec4f;
    fn fragment(&self, bar: Vec3f) -> (bool, TGAColor);
}

pub fn triangle(clip_verts: &[Vec4f], shader: &impl IShader, image: &mut TGAImage, zbuffer: &mut Vec<f32>, view_bundle: &ViewBundle) {
    let Viewport = view_bundle.ViewPort.clone();
    
    let pts = [Viewport.clone()*clip_verts[0], Viewport.clone()*clip_verts[1], Viewport.clone()*clip_verts[2]];
    let pts2 = [proj(pts[0]/pts[0][3]), proj(pts[1]/pts[1][3]), proj(pts[2]/pts[2][3])];

    let mut bboxmin = Vec2f::new_args(f32::MAX, f32::MAX);
    let mut bboxmax = Vec2f::new_args(f32::MIN, f32::MIN);
    let clamp = Vec2f::new_args((image.get_width()-1) as f32, (image.get_height()-1) as f32);
    for i in 0..3 {
        for j in 0..2 {
    ...
    for x in (bboxmin.x as i32)..=(bboxmax.x as i32) {
        for y in (bboxmin.y as i32)..=(bboxmax.y as i32) {
            ...
            let mut bc_clip = Vec3f::new_args(bc_screen.x/pts[0][3], bc_screen.y/pts[1][3], bc_screen.z/pts[2][3]);
            bc_clip = bc_clip/(bc_clip.x+bc_clip.y+bc_clip.z);
            let frag_depth = Vec3f::new_args(clip_verts[0][2], clip_verts[1][2], clip_verts[2][2])*bc_clip;
            if bc_screen.x<0. || bc_screen.y<0. || bc_screen.z<0. || frag_depth > zbuffer[(x+y*image.get_width()) as usize] { continue; }
